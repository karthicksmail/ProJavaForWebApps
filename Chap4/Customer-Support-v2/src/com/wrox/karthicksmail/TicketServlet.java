package com.wrox.karthicksmail;
...
@WebServlet(name = "ticketServlet", urlPatterns = { "/tickets-v2" }, loadOnStartup = 1)
...
public class TicketServlet extends HttpServlet {
	...
	private void showTicketForm(HttpServletRequest request,
		...
		request.getRequestDispatcher("WEB-INF/jsp/createTicket.jsp").forward(request, response);
	}
	...
	private void viewTicket(HttpServletRequest request,
		...
		String justCreated = request.getParameter("justCreated");
		if (ticket == null) {
			return;
		}
		if (justCreated == null) {
		...
		if (justCreated.equals("1")) {
			request.setAttribute("justCreated", new Boolean(true));
		} else {
			request.setAttribute("justCreated", new Boolean(false));
		}
	...
	private void listTickets(HttpServletRequest request, HttpServletResponse response)
		...
		request.getRequestDispatcher("WEB-INF/jsp/listTickets.jsp").forward(request, response);
	}
	...
	private void createTicket(HttpServletRequest request,
		...
		response.sendRedirect("tickets-v2?action=view&ticketId=" + id + "&justCreated=1");
	}
